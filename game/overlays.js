...
export const OVERLAYS = [
  ...
  { key: 'tax', kind: 'scalar', category: 'Economy', label: 'Tax burden', icon: 'coin',
    good: false, accessor: (isl) => num(isl.tax), vfmt: pct,
    domain: 'auto', skipEmpty: true, lo: 'light', hi: 'heavy' },
  ...
  { key: 'mood', kind: 'scalar', category: 'Society', label: 'Public mood', icon: 'spark',
    good: true, accessor: (isl) => (num(isl.approval) + 1) / 2, vfmt: (v) => (v >= 0.5 ? '+' : '') + Math.round((v * 2 - 1) * 100) + '%',
    domain: 'unit', skipEmpty: false, lo: 'restive', hi: 'pleased' },
  ...
  { key: 'corruption', kind: 'scalar', category: 'Order', label: 'Corruption (exposed)', icon: 'warning',
    good: false, accessor: (isl) => num(isl.corruption), vfmt: pct,
    domain: 'auto', skipEmpty: true, lo: 'clean', hi: 'venal' },
